...
class TGA(nn.Module):
    def __init__(self, scale):
        ...
        ng = 16
        pad = (0,1,1)
        conv3d_1 = nn.Conv3d(3, nf, (1,3,3))
        conv3d_1.apply(initialize_weights)
        self.weight1 = conv3d_1.weight
        self.bn3d_1 = nn.BatchNorm3d(nf, eps=1e-3, momentum=1e-3)

        conv3d_2 = nn.Conv3d(nf, nf, (1,3,3))
        conv3d_2.apply(initialize_weights)
        self.weight2 = conv3d_2.weight
        self.bn3d_2 = nn.BatchNorm3d(nf, eps=1e-3, momentum=1e-3)

        conv3d_2_1 = nn.Conv3d(nf, nf, (1,3,3))
        conv3d_2_1.apply(initialize_weights)
        self.weight3 = conv3d_2_1.weight
        self.bn3d_2_1 = nn.BatchNorm3d(nf, eps=1e-3, momentum=1e-3)

        conv3d_2_2 = nn.Conv3d(nf, nf, (3,3,3))
        conv3d_2_2.apply(initialize_weights)
        self.weight4 = conv3d_2_2.weight
    ...
    def forward(self, x, HR):
        ...
        g2 = x[:,:,3:6,:,:]
        g3 = x[:,:,6:9,:,:]
        g1 = F.relu(self.bn3d_1(nn.functional.conv3d(g1, self.weight1, stride=(1,1,1), padding=(0,3,3), dilation=(1,3,3))))
        g1 = F.relu(self.bn3d_2(nn.functional.conv3d(g1, self.weight2, stride=(1,1,1), padding=(0,3,3), dilation=(1,3,3))))
        g1 = F.relu(self.bn3d_2_1(nn.functional.conv3d(g1, self.weight3, stride=(1,1,1), padding=(0,3,3), dilation=(1,3,3))))
        g1 = nn.functional.conv3d(g1, self.weight4, stride=(3,1,1), padding=(0,3,3), dilation=(1,3,3))

        g2 = F.relu(self.bn3d_1(nn.functional.conv3d(g2, self.weight1, stride=(1,1,1), padding=(0,1,1), dilation=1)))
        g2 = F.relu(self.bn3d_2(nn.functional.conv3d(g2, self.weight2, stride=(1,1,1), padding=(0,1,1), dilation=1)))
        g2 = F.relu(self.bn3d_2_1(nn.functional.conv3d(g2, self.weight3, stride=(1,1,1), padding=(0,1,1), dilation=1)))
        g2 = nn.functional.conv3d(g2, self.weight4, stride=(3,1,1), padding=(0,1,1), dilation=1)

        g3 = F.relu(self.bn3d_1(nn.functional.conv3d(g3, self.weight1, stride=(1,1,1), padding=(0,2,2), dilation=(1,2,2))))
        g3 = F.relu(self.bn3d_2(nn.functional.conv3d(g3, self.weight2, stride=(1,1,1), padding=(0,2,2), dilation=(1,2,2))))
        g3 = F.relu(self.bn3d_2_1(nn.functional.conv3d(g3, self.weight3, stride=(1,1,1), padding=(0,2,2), dilation=(1,2,2))))
        g3 = nn.functional.conv3d(g3, self.weight4, stride=(3,1,1), padding=(0,2,2), dilation=(1,2,2))

        x = torch.cat((g1,g2,g3),dim=2)
        x = self.head(x)
        x = self.middle_1(x)
        x = self.middle_2(x)
        x = self.middle_3(x)
